...
class Forest:
    ...
    def union(self, pi, pi_edge, pj, pj_edge, forest):
        ...
        r = ((pi_edge + self.rotations[pi]) % 4 -
             (pj_edge + forest.rotations[pj]) % 4 + 2) % 4
        forest.set_position(pj, pj_P[0], pj_P[1], r)

        ideal_member_length = len(self.members + forest.members)
        real_member_length = len(
            set(list(self.positions.values()) + list(forest.positions.values())))
    ...
    def parallel_shift(self, dx, dy):
        for id in self.members:
            self.positions[id] = (self.positions[id][0] +
                                  dx, self.positions[id][1] + dy)
        return
...
class Solver:
    ...
    def run(self):
        while(len(self.queue)):
            ...
            if(self.parents[pi] == self.parents[pj]):
                continue
            forest, forest_root, forest_member = self.forests[self.parents[pi]].union(
                pi, pi_edge, pj, pj_edge, self.forests[self.parents[pj]])
            if forest_root == -1:
                continue
    ...
    def create_img(self, forest):
        ...
        for id, pos in forest.positions.items():
            ...
            pic[(pos[1]-minY)*self.size:(pos[1]-minY+1)*self.size, (pos[0]-minX)
                * self.size:(pos[0]-minX+1)*self.size] = self.pieces[id].img
...
if __name__ == "__main__":
    ...
    # print(col, row)
    pieces = [Piece(part, i) for i, part in enumerate(np.reshape([np.hsplit(h, row)
                                                                  for h in np.vsplit(img.astype(np.int16), col)], (-1, PIECE_SIZE, PIECE_SIZE, 3)))]
