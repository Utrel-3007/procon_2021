...
if __name__ == "__main__":
    ...
    pieces = [piece for piece in np.reshape([np.hsplit(h, width) for h in np.vsplit(
        img.astype(np.int16), height)], (-1, PIECE_SIZE, PIECE_SIZE, 3))]
    ...
    for y in range(height):
        for x in range(width):
            pic[y*PIECE_SIZE:(y+1)*PIECE_SIZE, x*PIECE_SIZE:(x+1)
                * PIECE_SIZE] = pieces[y*width+x]
...
#     img = cv2.imread(config.PROBLEM_NAME)
