...
class Solver:
    def __init__(self, data):
        ...
        self.now_col, self.now_row = np.unravel_index(
            np.argmax(data), data.shape)[::-1]
        self.solution = [hex(self.now_col)[2:].upper() +
                         hex(self.now_row)[2:].upper() + '\n']
    ...
    def venture(self, route, target, out=True, sig=True):
        for proc in route:
            if out:
    ...
    def setup(self, target):
        ...
        route = self.get_route((self.now_col, self.now_row),
                               (target_col, target_row + 1), (target_col, target_row))
        if target_row == self.height - 1:
            route = self.get_route(
                (self.now_col, self.now_row), (target_col, target_row - 1), (target_col, target_row)) + 'D'
    ...
    def run(self):
        ...
        if len(route):
            ...
            # print("Hello", route)
            # print("1", self.solution)
            self.solution.append(route)
            # print("2", self.solution)
            self.venture(route, target, sig=False)
...
if __name__ == "__main__":
    ...
    solution += Solver(np.loadtxt(config.POSITION_FILE, dtype=int)).run()

    # print(solution)

    # with open('solution.txt', 'w') as f:
    with open(config.SOLUTION_NAME, 'w') as f:
